...
def mock_sync_uri() -> URL:
    return PostgresSessionFactory.SyncSession.create_uri(
        host='localhost', port=5432, database='test_db', user='user', password='password'
    )
...
def test_sync_initialize(mock_sessionmaker, mock_create_engine, mock_psycopg_connect, mock_sync_uri: URL, cleanup):
