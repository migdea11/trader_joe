...
# now a module-scope dependency -- first the Alpaca client, then the Kafka producer -- made
# `import data.ingest.app.main` raise, and the suite stayed green because nothing anywhere imports
# an app main module. data.store.app.main has the same exposure: it reaches
# common.kafka.kafka_config and data.store.app.database.database, both of which read the
# environment while their module bodies run. uvicorn imports it in a fresh interpreter at startup,
...
# The ingest copy also asserts a module-scope RPC server count. That assertion is not portable to
# data_store and is deliberately absent here: it reads `rpc_module.rpc._rpc_servers`, and no
# data_store module has an `rpc` attribute to read. The only module-scope
# `rpc = KafkaRpcFactory(...)` in the repository is routers/data_ingest/get_dataset_request.py.
# data_store builds its factories inside functions -- data/store/app/app_depends.py lifespan() and
# routers/common/latency.py initialize_latency_client()/initialize_latency_server() -- so the
# expected count is 0 only in the sense that there is nothing to count; copying the line over would
# raise AttributeError, and `getattr(..., 'rpc', None)` would pass precisely because the attribute
# is missing, which is the vacuity the rest of this file is built to avoid. tj-4vnmik sanctions
# dropping it rather than inventing a number. RPC_MODULE is still imported by the probe, so an
# import-time break in the store's RPC-facing router is still caught.

# Every prefix that data.store.app.main's import closure reads at module scope, and so a
# per-service constant like APP_MODULE above -- not a tuple to copy across unchanged. BROKER_* is
# the Kafka one: common.kafka.kafka_config reads BROKER_NAME/BROKER_PORT/BROKER_CONN_TIMEOUT at
# import time, and BROKER_PORT is cast to int, which is the exact read that used to explode.
# DATABASE_* and POSTGRES_* are the store's own: data/store/app/database/database.py:12-17 reads
# DATABASE_NAME/DATABASE_PORT/POSTGRES_DB_NAME/POSTGRES_USER/POSTGRES_PASS/DATABASE_CONN_TIMEOUT
# while its body runs, reached from main by three separate chains, and DATABASE_PORT and
# DATABASE_CONN_TIMEOUT are cast to int. ALPACA_* carries over from the ingest copy and matches
# nothing in this closure; it is kept only so a credential that migrates here is caught on
# arrival. Omitting a prefix this service reads is what makes the last assertion vacuous.
CREDENTIAL_PREFIXES = ('ALPACA_', 'BROKER_', 'DATABASE_', 'KAFKA_', 'POSTGRES_')
